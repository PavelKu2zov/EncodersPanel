...
uint32_t crc32(uint8_t *msg_ptr, uint32_t byte_cnt, uint32_t oldchksum, bool init, bool last)
{
    ...
    remainder = (init == true) ? CRC32_INIT_REMAIDER : oldchksum;

    /* divide the messsage by the polynomial, one byte at a time */
    for (offset=0; offset<byte_cnt; offset++)
    {
        byte = (remainder >> (CRC32_WIDTH - 8)) ^ msg_ptr[offset];
